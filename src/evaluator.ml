...
let rec sub_lift_goals sub g =
    List.map (fun g1 -> sub_lift_goal sub g1) g
...
let rec occurs s t =
    ...
    | VarExp n -> (
        ...
        | VarExp m -> n = m
        | TermExp (st, el) ->
            List.fold_left (fun acc v -> acc || (occurs s v)) false el
        | _ -> false
    )
    | _ -> raise(Failure "not needed")
...
let print_res e orig_query_vars orig_vars_num =
    List.fold_right (
        fun env r2 ->
            print_string "====================\n";
            List.fold_right (
                fun d r -> (
                    ...
                    | VarExp v -> (
                        try let f = List.assoc (VarExp v) env in (
                            ...
                            | TermExp (st, el) ->
                                print_string (v ^ " = " ^ (
                                    string_of_atom ((TermExp(st,el))) ^ "\n"));
                                r
                            | VarExp v2 ->
                                print_string (v ^ " is free\n"); r
                            | ConstExp cv ->
                                print_string (v ^ " = " ^ (
                                    string_of_exp f) ^ "\n"); r
                        )
