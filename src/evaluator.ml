...
(* 
   uniq:
     * takes in:
         l - a list
     * returns a the lsit reversed with only one copy of each element
...
let rec rename_vars_in_dec d =
    ...
    | Clause (h, b) ->
        ...
        (* substite new names for variables *)
...
(* 
   pairandcat:
     * takes in:
         ...
         c - a list of constriants where each cosntraint is of type (exp * exp)
     * returns a new list of constraints where c is prepended with each entry
       ...
       to make a new contraint
     * used for implemneting decompose for unification
...
(* 
   replace:
     * takes in:
         c - a list of constraints where each cosntraint is of type (exp * exp)
         sub - a listof substitutions
...
(* 
   eval_query:
     ...
     * returns a list of lists of subsittuions where each 
       substitution is of type (exp * exp)
         - if the returned lsit is empty then no solutions were found for the 
           query for the given db
         - otherwise, each element is a list of substittutions for one solution
...
(* 
   string_of_res:
     * takes in:
         e - a list of lists of subsittuions where each 
             substitution is of type (exp * exp)
         orig_query_vars - a list of uniq VarExps that appeared 
                           in the orginal query
...
let string_of_res e orig_query_vars orig_vars_num =
   ...
   List.fold_left (
        ...
        then
          "====================\n" ^
            (* iterate over orignal query vars to find their subsittuion *)
            (List.fold_left (
                 fun r d -> (
                   match d with
                   | VarExp v -> (
